...
public class GoogleLocationUtil {
    private static final GeoApiContext.Builder placeBuilder = new GeoApiContext.Builder().apiKey(Constants.GOOGLE_API_KEY);
    private static final GeoApiContext placeContext = placeBuilder.build();

    private static final GeoApiContext.Builder builder = new GeoApiContext.Builder().apiKey(Constants.GOOGLE_API_KEY).disableRetries();
    private static final GeoApiContext context = builder.build();

    public static void main(String[] args) throws Exception {
        List<Double> location = getCoordinate("HUE, Vietnam");
    ...
    public static double calculateDistanceFrom2Point(LatLng p1, LatLng p2, String env) {
        ...
        try {
            ...
            System.err.println(String.format("calculateDistanceFrom2Point for (%s, %s) -> %s", p1, p2, e.toString()));
//            e.printStackTrace();
        }
